...
const GroupMenusDashboard = () => {
  ...
  const fetchGroupMenus = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:8000/group-menus/");
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://127.0.0.1:8000/group-menus/${id}`);
      fetchGroupMenus();
    } catch (error) {
  ...
  const currentItems = groupMenus.slice(indexOfFirstItem, indexOfLastItem);
      ...
      <Navbar fluid className="bg-transparent shadow-lg">
        <Navbar.Brand href={getDashboardLink()}>
          ...
          <span className="self-center whitespace-nowrap text-xl font-semibold text-white">
            Restaurant Admin Dashboard - Group Menus
          </span>
        </Navbar.Brand>
            ...
            <div className="overflow-x-auto mt-8">
              <Table hoverable>
                <Table.Head>
                  ...
                  <Table.HeadCell>Menu Items</Table.HeadCell>
                ...
                <Table.Body className="divide-y">
                  {currentItems.map((groupMenu) => (
                    ...
                    >
                      ...
                      <Table.Cell>{groupMenu.description}</Table.Cell>
                      <Table.Cell>{groupMenu.price}KM</Table.Cell>
                      <Table.Cell>
                        <ul>
                      ...
                      <Table.Cell>
                        <div className="flex justify-end space-x-2">
                          <Link to={`/edit-group-menu/${groupMenu.id}`}>
                          ...
                          <Button
                            ...
                            onClick={() => handleDelete(groupMenu.id)}
                          >
                            Delete
                          </Button>
                        </div>
              ...
              <div className="flex justify-center mt-4">
                <Pagination
                  ...
                  totalPages={Math.ceil(groupMenus.length / itemsPerPage)}
...
export default GroupMenusDashboard;
