...
const EditRestaurantTypePage = () => {
  ...
  useEffect(() => {
    ...
    fetchRestaurantTypeDetails();
  }, [id]);

  const fetchRestaurantTypeDetails = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/restaurant-types/${id}`);
      const restaurantType = response.data;
      setName(restaurantType.name);
    } catch (error) {
      console.error("Error fetching restaurant type details:", error);
    }
  };

  const handleSave = async () => {
    try {
      const response = await axios.put(`http://127.0.0.1:8000/restaurant-types/${id}`, {
        name,
      });

      if (response.status === 200) {
        setSuccessMessage("Saved successfully!");
        navigate("/restaurant-types");
      }
    } catch (error) {
      ...
      <Navbar fluid className="bg-transparent shadow-lg">
        <Navbar.Brand href="/admin-dashboard">
          ...
          <span className="self-center whitespace-nowrap text-xl font-semibold text-white">
            Admin Dashboard - Edit Restaurant Type
          </span>
        </Navbar.Brand>
        ...
        <div className="p-8 flex-grow login-signup-container">
          <div className="login-form-container">
            <div className="add-restaurant-card">
              <h2 className="mb-6 block">Edit Restaurant Type</h2>
              <form className="flex flex-col gap-0">
                <div className="mb-4 block">
                  <Label htmlFor="name" value="Restaurant Type Name" />
                  <TextInput
                    id="name"
                    type="text"
                    value={name}
...
export default EditRestaurantTypePage;
